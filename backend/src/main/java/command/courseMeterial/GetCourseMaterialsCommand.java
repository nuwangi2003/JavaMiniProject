...
public class GetCourseMaterialsCommand implements Command {
    ...
    public GetCourseMaterialsCommand(CourseMaterialService courseMaterialService, AuthService authService) {
    ...
    public void execute(Object data, ClientContext context) {
        ...
        try {
            ...
            if (!"Lecturer".equalsIgnoreCase(context.getRole())) {
                response.put("success", false);
                response.put("message", "Only lecturers can view course materials.");
                context.getOutput().println(mapper.writeValueAsString(response));
                return;
            }
            ...
            List<?> materials =
                    courseMaterialService.getMaterialsByCourse(courseId, context.getUserId());
