...
public class GetMyGPACommand implements Command {

    private final GPAService service;
    private final ObjectMapper mapper = new ObjectMapper();

    public GetMyGPACommand(GPAService service) {
    ...
    public void execute(Object data, ClientContext context) {
        try {
            ...
            GPAReport report = service.getMyLatestGPA(context.getUserId());
            MyDataResponseDTO response = new MyDataResponseDTO(
                    report != null,
                    report != null ? "My GPA retrieved." : "No GPA records found.",
                    report
            );
